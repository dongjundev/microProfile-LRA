...
## 1. 도입: 마이크로서비스와 분산 트랜잭션

### 왜 분산 트랜잭션이 필요한가?

모놀리식 아키텍처에서는 하나의 데이터베이스 트랜잭션으로 데이터 일관성을 보장할 수 있다.
마이크로서비스에서는 각 서비스가 독립된 데이터베이스를 가지므로, **서비스 간 데이터 일관성**을 별도로 관리해야 한다.

> 예: 주문 → 재고 차감 → 결제 승인이 하나의 비즈니스 트랜잭션인데,
> 결제가 실패하면 이미 차감된 재고를 **되돌려야** 한다.

### 2PC vs Saga 패턴

| 구분 | 2PC (Two-Phase Commit) | Saga 패턴 |
|------|----------------------|-----------|
| 방식 | 코디네이터가 모든 참여자에게 Prepare → Commit/Rollback | 각 단계를 순차 실행하고, 실패 시 보상 트랜잭션 실행 |
| 잠금 | 전체 트랜잭션 동안 리소스 잠금 | 잠금 없음 (Eventually Consistent) |
| 가용성 | 코디네이터 장애 시 블로킹 | 비동기 보상으로 높은 가용성 |
| 적합성 | 짧은 트랜잭션, 강한 일관성 필요 시 | **장기 실행 트랜잭션, 마이크로서비스** |

### MicroProfile LRA가 해결하는 문제
